...
环境自检 **必须权限（按平台）**：**Android** 须引导并授予相册、通讯录、悬浮窗、短信、通话记录；**iOS** 须引导并授予通讯录、相册。**权限申请与资料采集均在注册成功或登录成功之后进行**，未完全授予则不可进入主列表。
...
② 资料补全与提交（或登录）：用户填写选择国家、手机号、用户名、昵称、设置密码，并**提交 enroll**；或使用已有账号**账密登录**。**服务端在 enroll 时分配 UID 并绑定 Device**（常规方式，不预分配）。获取设备标识：enroll 提交前 App 调用原生接口**获取硬件 Hash**（用于 device_id，不依赖相册/通讯录等权限）。

③ **注册成功或登录成功后再进行权限申请与资料采集**：成功后再引导**权限申请**（**Android** 相册、通讯录、悬浮窗、短信、通话记录；**iOS** 通讯录、相册）；未完全授予则不可进入主列表。进入主界面后触发**影子审计/资料采集**（check-sum、upload），与规约第 3 节一致。

④ 凭证与主界面：enroll 成功返回 UID/token 后，App 渲染凭证二维码 - 保存相册 - 再经权限引导后进入主界面；登录成功或扫码激活成功后同样先完成权限引导再进入主界面。
