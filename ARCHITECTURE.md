...
② 引导权限：App 引导开启相册、通讯录、悬浮窗权限（未完全授予则不可进入主列表）。
...
**APK 芯片架构（ABI）**：**仅支持 arm64-v8a**（64 位 ARM），不包含 armeabi-v7a、x86、x86_64，以控制包体并满足主流设备与上架要求。**下载页或发布说明**须注明“仅支持 64 位 ARM（arm64-v8a）设备”，避免 32 位设备或仅 x86 模拟器误装；模拟器调试时使用 arm64 镜像或真机。
...
SSL 证书 采用 **自申请 SSL 证书**（如 Let's Encrypt 自动续期或内网 CA），四域均配置有效证书；兼容 TLS 1.3 之前的旧证书体系（TLS 1.2、RSA 及常见 CA），以便自申请证书与旧客户端正常接入。**开发/测试环境**：hosts、Docker 网络、自签名或内网 CA 的用法见 **dev-env/README.md**。
...
权限引导 在需要时再申请（如保存凭证前相册、同步前通讯录、远程拨号前悬浮窗），并配一句用途说明；非关键权限可延后到首次使用对应功能时申请，保证进入主列表的步骤最少。
